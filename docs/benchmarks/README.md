...
- [`accuracy.md`](accuracy.md) - does fanning the same request out to multiple (often small/cheap) models and having Jev pick a winner actually produce a better answer than a single model call? Methodology defined; results not yet collected.

## Planned

- A JudgeJev-level concurrency/latency benchmark: end-to-end request time through the real proxy (fan-out + rubric selection + judging), not just the underlying inference server - to separate "how fast is the model" from "how much overhead does JudgeJev itself add."
