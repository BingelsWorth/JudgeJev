...
- `npx tsx test-integration.mjs` run against the real local vLLM box (2026-09-19): single request, 5x fan-out, and live Jev judging all pass. This surfaced and fixed real bugs the mocked tests couldn't catch:
  - The vLLM box is a reasoning model (Qwen3) that emits a `<think>...</think>` block before answering. The first fix disabled thinking (`chat_template_kwargs: {enable_thinking:false}`) to avoid burning the token budget on it - correctly identified as the wrong call (reasoning is genuinely useful for judgment quality, and the flag is vLLM/Qwen3-specific, not portable to a real Jev service). Replaced with: `callJevJudge` now requests a streamed completion internally (still no streaming on the downstream JudgeJev API - this is purely an implementation detail of the one internal call) and cancels the connection the instant a syntactically complete, parseable verdict appears after the last `</think>`, so the full reasoning budget stays available but latency tracks actual thinking length rather than a fixed ceiling. `max_tokens` is now a generous safety cap (default 4000, overridable) for the case where no verdict ever appears.
  - `test-integration.mjs` itself had a pre-existing bug where the fan-out test's routes never matched `state.models` (silently resolved to 0 workers every run) and the single-request test never sent a `model` field at all (silently omitted by `JSON.stringify`, only "working" because the box falls back to its one loaded model) - both fixed to use the real served model name (`Qwen/Qwen3-1.7B`, override via `VLLM_MODEL`).
